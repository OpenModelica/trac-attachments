...
// dummy equation
    ...
    model test
      ...
      HeatedTank_2019.Basic.MyGlobalRandom RNG(useInitialSeed = true, globalSeed = seedvalue) annotation(
      ...
      // on a une entrée réelle (qui sera donnée par Openturns) qui sert à calculer une graine imposée (cf. 1ère équation)
      // et deux sorties de type variables indicatrices.
      input Real X = 3.14;
      Integer seedvalue;
      output Boolean parallel;
      output Boolean series;
    equation
      seedvalue = integer(abs(ceil(X)));
      parallel = and1.y;
      series = or1.y;
      ...
      connect(RNG.y, block2.u) annotation(
        Line(points = {{-59, -28}, {-46, -28}, {-46, 12}, {-80, 12}, {-80, 24}, {-74, 24}}, color = {255, 127, 0}));
      connect(block1.u, block2.u) annotation(
        Line(points = {{-74, 62}, {-80, 62}, {-80, 24}, {-74, 24}}, color = {255, 127, 0}));
      annotation(
        Diagram(graphics = {Text(origin = {63, 56}, extent = {{-39, 6}, {39, -6}}, textString = "Parallel system", lineColor = {0, 0, 0}), Text(origin = {55, 3}, extent = {{-25, 7}, {31, -11}}, textString = "Series system", lineColor = {0, 0, 0})}, coordinateSystem(initialScale = 0.1)),
    ...
    model MyGlobalRandom "Global seed and random number function (produces random number in the range 0 < number <= 1)"
      ...
      parameter Boolean useInitialSeed = false "= true, if reproducible sequence with the given seed" annotation(
        choices(checkBox = true));
      input Integer globalSeed = 12345 "Seed to initialize random number generator";
      Integer seed;
...
// inspired by the equation of block hysteresis
    ...
    model system
      ...
      HeatedTank_2019.HeatedTank_from_FormL.InterfaceTankToHydrodevice interfaceTankToHydrodevice1(pre_command_start = true) annotation(
        Placement(visible = true, transformation(origin = {52, -16}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      HeatedTank_2019.HeatedTank_from_FormL.InterfaceTankToHydrodevice interfaceTankToHydrodevice2(pre_command_start = true) annotation(
    ...
    equation
      ...
      connect(interfaceTankToHydrodevice1.y, not1.u) annotation(
        Line(points = {{63, -16}, {68, -16}, {68, -82}, {-84, -82}, {-84, -44}, {-72, -44}}, color = {255, 0, 255}));
      connect(interfaceTankToHydrodevice2.y, pump_1.u) annotation(
        Line(points = {{63, 6}, {82, 6}, {82, 80}, {-70, 80}, {-70, 60}, {-58.8, 60}}, color = {255, 0, 255}));
      annotation(
        experiment(StartTime = 0, StopTime = 100, Tolerance = 1e-06, Interval = 0.2),
 ...
 value of control components")}, coordinateSystem(initialScale = 0.1)));
  ...
  package HeatedTank_with_failures
    model HydroDevice
      ...
      Real v_c;
      type State = enumeration(on, off, stuck_on, stuck_off);
      State state(start = State.on, fixed=true) annotation(
        Dialog(enable = true));
      ...
      Modelica.Blocks.Interfaces.BooleanInput u annotation(
        Placement(visible = true, transformation(origin = {-120, 0}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-120, 0}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
      Basic_woGraph.Block Stuck_open(independent = false, failureRate = 0.02, repairRate = 0) annotation(
        Placement(visible = true, transformation(origin = {-38, 42}, extent = {{-18, -18}, {18, 18}}, rotation = 0)));
      HeatedTank_2019.Basic_woGraph.Block Stuck_closed(independent = false, failureRate = 0.02, repairRate = 0) annotation(
      ...
      Modelica.Blocks.Logical.Or or1 annotation(
        Placement(transformation(extent = {{16, 2}, {36, 22}})));
    initial equation
      pre(state) = State.on;
    algorithm
      if pre(state) == State.on and u == false then
...
// Sharing the seed of RNG
      ...
      y = -v_c annotation(
        Diagram(coordinateSystem(initialScale = 0.1)),
        Icon(graphics = {Ellipse(origin = {7, 1}, fillColor = {255, 85, 255}, fillPattern = FillPattern.Solid, extent = {{-89, 73}, {89, -73}}, endAngle = 360)}));
      connect(Stuck_open.y, or1.u1) annotation(
        Line(points = {{-18.2, 42}, {-6, 42}, {-6, 40}, {4, 40}, {4, 12}, {14, 12}}, color = {255, 0, 255}));
      connect(Stuck_closed.y, or1.u2) annotation(
        Line(points = {{-18.2, -28}, {4, -28}, {4, 4}, {14, 4}}, color = {255, 0, 255}));
      connect(or1.y, Stuck_closed.FailureIsPresent) annotation(
        Line(points = {{37, 12}, {42, 12}, {42, -58}, {-68, -58}, {-68, -31.6}, {-56, -31.6}}, color = {255, 0, 255}));
      connect(or1.y, Stuck_open.FailureIsPresent) annotation(
        Line(points = {{37, 12}, {42, 12}, {42, 58}, {-72, 58}, {-72, 38.4}, {-56, 38.4}}, color = {255, 0, 255}));
      connect(Stuck_open.FailureIsPresent, Stuck_closed.FailureIsPresent) annotation(
        Line(points = {{-56, 38.4}, {-80, 38.4}, {-80, -31.6}, {-56, -31.6}}, color = {255, 0, 255}));
      annotation(
        Icon(graphics = {Ellipse(origin = {6, 0}, fillColor = {255, 0, 255}, fillPattern = FillPattern.Solid, extent = {{-80, 62}, {80, -62}}, endAngle = 360), Text(origin = {-23, 27}, extent = {{93, -87}, {-35, 37}}, textString = "%name")}, coordinateSystem(initialScale = 0.1)),
...
// inspired by the equation of block hysteresis
    ...
    model system
      ...
      HeatedTank_2019.HeatedTank_with_failures.Tank tank annotation(
        Placement(visible = true, transformation(origin = {-8, 4}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
      HeatedTank_2019.HeatedTank_with_failures.InterfaceTankToHydrodevice interfaceTankToHydrodevice1(pre_command_start = true) annotation(
        Placement(visible = true, transformation(origin = {52, -16}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      HeatedTank_2019.HeatedTank_with_failures.InterfaceTankToHydrodevice interfaceTankToHydrodevice2(pre_command_start = false) annotation(
        Placement(visible = true, transformation(origin = {52, 6}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    equation
      ...
      connect(interfaceTankToHydrodevice1.y, not1.u) annotation(
        Line(points = {{63, -16}, {70, -16}, {70, -70}, {-82, -70}, {-82, -46}, {-76, -46}}, color = {255, 0, 255}));
      connect(not1.y, valve_1.u) annotation(
...
//perfect component
    ...
    model test
      HeatedTank_2019.Basic_woGraph.Block block1(perfect = false, independent = false, failureRate = 0.1) annotation(
        Placement(visible = true, transformation(origin = {-64, 62}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      HeatedTank_2019.Basic_woGraph.MyGlobalRandom RNG(useInitialSeed = false) annotation(
        Placement(visible = true, transformation(origin = {-84, 86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      HeatedTank_2019.Basic_woGraph.Block block2(independent = false, failureRate = 0.2) annotation(
      ...
      Block Independent_failure(failureRate = 0.1) annotation(
        Placement(transformation(extent = {{-72, -70}, {-30, -28}})));
    equation
      RNG.y = block1.u;
      RNG.y = block2.u;
      RNG.y = Independent_failure.u;
      connect(block1.y, or1.u1) annotation(
        Line(points = {{-53, 62}, {-2, 62}, {-2, 0}}, color = {255, 0, 255}));
      connect(block1.y, and1.u1) annotation(
        Line(points = {{-53, 62}, {6, 62}, {6, 54}}, color = {255, 0, 255}));
      connect(block2.y, or1.u2) annotation(
        Line(points = {{-55, 16}, {-38, 16}, {-38, -8}, {-2, -8}}, color = {255, 0, 255}));
      connect(block2.y, and1.u2) annotation(
        Line(points = {{-55, 16}, {-22, 16}, {-22, 46}, {6, 46}}, color = {255, 0, 255}));
      connect(block1.FailureIsPresent, block2.FailureIsPresent) annotation(
        Line(points = {{-74, 60}, {-84, 60}, {-84, 14}, {-76, 14}}, color = {255, 0, 255}));
      connect(block1.y, or3.u[1]) annotation(
        Line(points = {{-53, 62}, {-52, 62}, {-52, 42.1}, {-46, 42.1}}, color = {255, 0, 255}));
      connect(block2.y, or3.u[2]) annotation(
        Line(points = {{-55, 16}, {-50, 16}, {-50, 37.9}, {-46, 37.9}}, color = {255, 0, 255}));
      connect(or3.y, block1.FailureIsPresent) annotation(
        Line(points = {{-33.1, 40}, {-28, 40}, {-28, 72}, {-78, 72}, {-78, 60}, {-74, 60}}, color = {255, 0, 255}));
      annotation(
        Diagram(graphics = {Text(origin = {59, 57}, extent = {{-35, 5}, {35, -5}}, textString = "Series system", lineColor = {0, 0, 0}), Text(origin = {55, 3}, extent = {{-25, 7}, {31, -11}}, textString = "Parallel system")}, coordinateSystem(initialScale = 0.1)),
        experiment(StartTime = 0, StopTime = 100, Tolerance = 1e-06, Interval = 0.2));
    end test;
