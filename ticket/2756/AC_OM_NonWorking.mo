package AC_OM_NonWorking
  model GridOneTrack "One track variable numbers of ESS's and trains, with inner grid (no commutator)"
    ...
    Modelica.Blocks.Interfaces.RealInput trainPos[numOfTrains] "each train's position" annotation(Placement(transformation(extent = {{-20, 20}, {20, -20}}, rotation = 0, origin = {-180, 0}), iconTransformation(extent = {{-15, -15}, {15, 15}}, origin = {-175, 1})));
    ...
    parameter Real iniSecPos[:] = {580, 1744, 3141} "Positions of section borders (typically ESS's abscissas)";
    parameter Real lineLength = 3500 "total length inclusind terminal trunks";
    final parameter Integer numOfSect = size(iniSecPos, 1) "Number of sections (typically number of ESS's)";
    parameter Modelica.SIunits.Resistivity resistivity[numOfSect + 1] = {0.000108, 0.000108, 0.00014, 0.00014} "resistivity of line trunks";
    //Il numero di resistenze da mmettere lungo la linea ? pari al numero di
    // sezioni + 1 in quanto possono essere differenti le resistivit? delle due mezze
    // sezioni di estremit?.
    /*Il numero di resistenze ? pari a :
    ...
    SimpleVarResis resisLeft[numOfSect] "Left section-side resistor", resisRight[numOfSect] "Right section-side resistor";
    /* Nota: resisRight[1] ? la resistenza del primo tratto a sbalzo; resisLeft[1])
                                       ? la resistenza di sinistra della prima sezione completa resisRight[2] ? la 
                                       resistenza di destra della prima sezione completa. 
                                       Inoltre il pin positivo di tutti i resistori ? messo a sinistra.
                                       In tal modo dentro la sezione resisLeft[i].n ? connesso a resisRight[i+1].p, 
                                       e ai confini di sezione resisRight[i].n ? connesso a resisLeft[i].p, 
                                       */
    SimpleVarResis resisConn[numOfTrains, numOfSect + 1];
    Integer z[numOfTrains];
    Modelica.SIunits.Power lostPower, singleLostPower[numOfSect];
  ...
  equation
    ...
    /*Calcolo i valori delle resistenze. In questa versione preliminare divido equamente
                               ...
                               della sola sezione in cui vi ? il treno.
                               La numerazione ? conforme a quanto riportato nel file "Schema rete.docx".
                              */
    for i in 1:numOfSect - 1 loop
    ...
    for i in 1:numOfSect loop
      singleLostPower[i] = resisLeft[i].v * resisLeft[i].i;
    end for;
    lostPower = sum(singleLostPower);
    /*  algorithm
                            lostPower := 0;
                            for i in 1:numOfSect loop
    ...
    annotation(Documentation(info = "<html>
     ...
     </html>"), Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-160, -60}, {160, 60}}), graphics), Icon(coordinateSystem(extent = {{-160, -60}, {160, 60}}, preserveAspectRatio = false), graphics = {Rectangle(extent = {{-160, 60}, {160, -60}}, lineColor = {0, 0, 255}, fillColor = {215, 215, 215}, fillPattern = FillPattern.Solid), Line(points = {{-158, 2}, {158, 2}}, color = {135, 135, 135}), Rectangle(lineColor = {135, 135, 135}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-116, 8}, {-96, -2}}), Rectangle(lineColor = {135, 135, 135}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-70, 8}, {-50, -2}}), Rectangle(lineColor = {135, 135, 135}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{50, 8}, {70, -2}}), Rectangle(lineColor = {135, 135, 135}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{98, 8}, {118, -2}}), Text(extent = {{-160, -12}, {158, -26}}, lineColor = {0, 0, 0}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "%numOfTrains trains"), Text(extent = {{-160, -34}, {160, -44}}, lineColor = {0, 0, 0}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "ESS's in %iniSecPos"), Text(extent = {{-158, 42}, {158, 24}}, lineColor = {0, 0, 255}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "%name")}));
  end GridOneTrack;

  model gridTest21
    GridOneTrack gridOneTrack(iniSecPos = {580, 1744}, lineLength = 2500, resistivity = {0.000108, 0.000108, 0.00014}, numOfTrains = 1) annotation(Placement(transformation(extent = {{-20, -20}, {80, 20}})));
    Modelica.Electrical.Analog.Basic.Ground ground annotation(Placement(transformation(extent = {{20, -88}, {40, -68}})));
    ...
    Modelica.Electrical.MultiPhase.Basic.PlugToPin_p tTn(m = 1, k = 1) annotation(Placement(visible = true, transformation(origin = {30, -32}, extent = {{10, -10}, {-10, 10}}, rotation = 90)));
    ...
    Modelica.Blocks.Sources.Ramp ramp1(height = 3300, duration = 1) annotation(Placement(visible = true, transformation(origin = {-60, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(ramp1.y, gridOneTrack.trainPos[1]) annotation(Line(points = {{-49, 0}, {-28.4047, 0}, {-28.4047, -1.16732}, {-28.4047, -1.16732}}, color = {0, 0, 127}));
    connect(tr1.n, ground.p) annotation(Line(points = {{30, -60}, {30, -68}}));
    connect(tTn.pin_p, tr1.p) annotation(Line(points = {{30, -34}, {30, -38}, {30, -40}}, color = {0, 0, 255}));
    connect(tTn.plug_p, gridOneTrack.tp) annotation(Line(points = {{30, -30}, {30, -30}, {30, -20}}, color = {0, 0, 255}));
    ...
    connect(sTn2.plug_p, gridOneTrack.ep) annotation(Line(points = {{30, 36}, {30, 20}}, color = {0, 0, 255}, smooth = Smooth.None));
    connect(sTn1.plug_p, gridOneTrack.ep) annotation(Line(points = {{-2, 36}, {-2, 28}, {30, 28}, {30, 20}}, color = {0, 0, 255}, smooth = Smooth.None));
    annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics), experiment(StartTime = 0, StopTime = 100, Tolerance = 1e-06, Interval = 0.2));
  end gridTest21;
