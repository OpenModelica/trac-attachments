...
model column1
 ...
 //Reboiler equation
  ...
  feed[1].P = 101325;
  feed[1].T = 370;
  feed[1].mixMolFlo = 100;
  feed[1].mixMolEnth = -2584.65;
  feed[1].mixMolEntr = 0;
  feed[1].mixMolFrac = {0.5, 0.5};
  feed[1].vapPhasMolFrac = 0;
  ...
  for i in 1:(noOfStages - 2) loop
    for j in 1:noOfFeeds loop
      if (feedStages[j] == i) then
        tray[i].feed.P := feed[j].P;
        tray[i].feed.T := feed[j].T;
        tray[i].feed.mixMolEntr := feed[j].mixMolEntr;
        tray[i].feed.vapPhasMolFrac := feed[j].vapPhasMolFrac;
//        tray[i].feed.mixMolFlo := feed[j].mixMolFlo;
//        tray[i].feed.mixMolEnth := feed[j].mixMolEnth;
//        tray[i].feed.mixMolFrac[:] := feed[j].mixMolFrac;        
        break;
      else
        tray[i].feed.P := 0;
        tray[i].feed.T := 0;
...
//        tray[i].feed.mixMolFlo := 0;
//        tray[i].feed.mixMolEnth := 0;
//        tray[i].feed.mixMolFrac[:] := zeros(NOC);
      end if;
    end for;
  end for;
  ...
  //tray equations for feed- mixMolFlo, mixMolFrac and mixMolEnth when written into if-else show that model is singular when they are written in algorithm it works fine.    
end column1;
