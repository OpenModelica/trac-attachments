package Control
        ...
        package Functions
                ...
                function CalcDiscreteTrapezium // Doesn't work on OpenModelica, but it does on Dymola. Try on newer versions of OpenModelica!
                         input Real acc1;
                         input Real acc2;
                         input Real dec1;
                         input Real dec2;
        ...
        package Servos // This package is obsolete. Use Controllers.MicroController instead!!!
        ...
        package Sources
                ...
                model DiscreteSpeedTrapezium
                      ...
                      output Real y;
                equation
                        dat = Functions.CalcDiscreteTrapezium(acc1,acc2,dec1,dec2,pv_acc,pv_dec,adv,vel,period);
                        accCount = dat[1];
                        decCount = dat[2];
                        remaining = dat[3];
                        when {initial(), sampleTrigger} then
                             y = if counter < accCount then Functions.AccelRamp(acc1,acc2,pv_acc,vel,counter*period)
                                 else if counter < accCount+decCount then Functions.DecelRamp(dec1,dec2,pv_dec,vel,(accCount+decCount-counter)*period)
                                 else 0;
                             counter = pre(counter)+1;
                        end when;
                end DiscreteSpeedTrapezium;
