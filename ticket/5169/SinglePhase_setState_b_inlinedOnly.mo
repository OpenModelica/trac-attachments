model SinglePhase_setState_b_inlinedOnly
  ...
  /*
    ...
    */
  Modelica.Blocks.Sources.Ramp p_sub(duration = 4, startTime = 0.1, height = pcrit - pmin, offset = pmin) annotation(
    Placement(transformation(extent = {{-80, 60}, {-60, 80}})));
  Modelica.Blocks.Sources.Ramp p_super(duration = 5, startTime = 6, offset = 0, height = pmax - pcrit) annotation(
    Placement(transformation(extent = {{-80, 20}, {-60, 40}})));
  Modelica.Blocks.Sources.Sine T_sine(freqHz = 100, startTime = 0, amplitude = (Tmax - Tmin) / 2, offset = (Tmax - Tmin) / 2 + Tmin) annotation(
...
end SinglePhase_setState_b_inlinedOnly;
