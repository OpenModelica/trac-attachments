...
partial model FlexiblePipeRC
  ...
  parameter SI.Temperature Toil = 273.15 + 20 "Oil temperature";
  ...
  final parameter SI.Density rhoOil_start = 1000 "start oil density";
  ...
  final parameter SI.BulkModulus Koil = 2e9 "oil bulk modulus";
  final parameter SI.KinematicViscosity kinVisc = 8.17e-6 "oil kinematic viscosity";
  ...
  SI.Density rho[N](each start = rhoOil_start, each fixed = true) "oil density profile, one value for each segment";  
  SI.Volume V[N](each start = A*L/N, each fixed = true) "pipe volume profile, one value for each segment";  
  SI.MassFlowRate w[N+1] "mass flowrate profile, one value for each node";
  SI.Pressure p[N](start = linspace(pP_start, pT_start, N), each fixed = true) "pressure profile, one value for each segment";
  ...
  ODF_P fl_P annotation(
      Placement(visible = true, transformation(origin = {-102, 0}, extent = {{-20, -10}, {20, 10}}, rotation = 0), iconTransformation(origin = {-100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
equation
  ...
  for i in 1:N loop
    M[i] = V[i]*rho[i];
    der(M[i]) = w[i] - w[i+1];
    der(V[i]) = A*dl*der(p[i])/Kpipe;
    if oilCompressibility then
      der(rho[i]) = rho[i]*der(p[i])/Koil;
    else
      rho[i] = rhoOil_start;
    end if;
  end for;
