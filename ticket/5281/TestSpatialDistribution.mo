within ;
package TestSpatialDistribution
  model Test1
  ...
  equation
   ...
   annotation(experiment(stopTime = 2, Interval = 1e-3, Tolerance = 1e-7));
  end Test1;
  ...
  equation
   ...
   (out0, out1) = spatialDistribution(in0, in1, x, v > 0, {0, 1}, {z0, z0});
    der(err0) = (out0_exact - out0)^2;
    der(err1) = (out1_exact - out1)^2;
    assert(err0 < 1e-3, "Numerical solution too far from exact one");
    assert(err1 < 1e-3, "Numerical solution too far from exact one");
